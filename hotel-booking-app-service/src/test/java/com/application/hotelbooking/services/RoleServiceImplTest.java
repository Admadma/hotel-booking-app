...
public class RoleServiceImplTest {
    ...
    public static final List<String> ROLE_NAMES = List.of("NAME2");
    public static final List<Role> ROLES = List.of(Role.builder().name("NAME1").build(), Role.builder().name("NAME2").build());
    public static final List<RoleModel> ROLE_MODELS = List.of(RoleModel.builder().name("NAME1").build(), RoleModel.builder().name("NAME2").build());
    @InjectMocks
...
//    }
    ...
    public void testGetRolesShouldReturnCollectionOfRolesWhereNameIsInTheList(){
        ...
        when(roleTransformer.transformToRoleModels(ROLES)).thenReturn(ROLE_MODELS);

        Collection<RoleModel> roleModels = roleService.getRoles(ROLE_NAMES);

        verify(roleRepository).findAll();
        verify(roleTransformer).transformToRoleModels(ROLES);
        Assertions.assertThat(roleModels).isNotNull();
        Assertions.assertThat(roleModels.size()).isEqualTo(1);
    }

    @Test
    public void testGetRolesSimplerShouldReturnCollectionOfRolesWhereNameIsInTheList(){
        when(roleRepository.findByNameIn(ROLE_NAMES)).thenReturn(ROLES);
        when(roleTransformer.transformToRoleModels(ROLES)).thenReturn(ROLE_MODELS);
